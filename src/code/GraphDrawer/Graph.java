...
public class Graph {
    ...
    public static BufferedImage createGraph(int height, int width, String textFunction, Domain domain) throws InvalidFunctionException {
        ...
        DepreciatedFunction depreciatedFunction;
        depreciatedFunction = new DepreciatedFunction(textFunction);

        double temp1 = depreciatedFunction.getValueFor(pivot.pixelsToX(0));
        double temp2;
        for(int i = 0; i < (width/defaultPrecision); i++) {
            temp2 = depreciatedFunction.getValueFor(pivot.pixelsToX(i*defaultPrecision+defaultPrecision));
